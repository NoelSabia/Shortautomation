...
class GPTCaller:
	def __init__(self, api_key: str, default_path: str) -> None:
		self.client_ = OpenAI(api_key=api_key)
		self.default_path_ = default_path
	...
	def rewrite(self, scraped_str: str, output_path: str = None) -> str:
		...
		if output_path is None or output_path == "":
			output_path = self.default_path_
		...
		while True:
			completion = self.client_.chat.completions.create(
				...
				messages=[
					{"role": "developer",
					 "content": "You are a professional writer and you have been given the task to rewrite the following text in a factual manner."},
					{
						"role": "user",
