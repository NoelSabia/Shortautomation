	...
	def signal_handler(self, sig, frame):
		...
		for directory in self._directory:
			try:
				...
				print(Fore.GREEN + f"Deleted directory: {directory}" + Style.RESET_ALL)
			except FileNotFoundError:
				print(Fore.YELLOW + f"Directory not found: {directory}, skipping." + Style.RESET_ALL)
			except PermissionError:
				print(Fore.RED + f"Insufficient permissions to delete: {directory}" + Style.RESET_ALL)
...
def main() -> None:
    ...
    for i, path in enumerate(master_org._directory):
        print(f"\nRunning main.py with path={path} and websites={master_org._websites[i]}")
        websites = master_org._websites[i]
        command = ["python3", "main.py", path] + websites
